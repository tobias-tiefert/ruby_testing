...
describe BinaryGame do
  ...
  describe '#player_input' do
    ...
    context 'when user inputs an incorrect value once, then a valid input' do
      before do
        allow(game_input).to receive(:gets).and_return('e', '3')
      end
    ...
    context 'when user inputs two incorrect values, then a valid input' do
      before do
        allow(game_input).to receive(:gets).and_return('e', '208', '3')
      end
  ...
  describe '#verify_input' do
    ...
    subject(:game_verify_input) { described_class.new(1, 10) }
    context 'when given a valid input as argument' do
      it 'returns valid input' do
        input = 3
        min = game_verify_input.instance_variable_get(:@minimum)
        max = game_verify_input.instance_variable_get(:@maximum)
        expect(game_verify_input.verify_input(min, max, input)).to eq(3)
      end
    end
    ...
    context 'when given invalid input as argument' do
      it 'returns nil' do
        ...
        min = game_verify_input.instance_variable_get(:@minimum)
        max = game_verify_input.instance_variable_get(:@maximum)
        expect(game_verify_input.verify_input(min, max, input)).to be_nil
      end
    end
  end
  ...
  describe '#display_turn_order' do
    ...
    let(:search_display_turn) { instance_double(BinarySearch) }
    before do
      # You'll need to create a few method stubs.
      allow(search_display_turn).to receive(:make_guess).and_return(3)
      allow(search_display_turn).to receive(:guess).and_return(3)
      allow(search_display_turn).to receive(:min).and_return(1)
      allow(search_display_turn).to receive(:max).and_return(10)
      allow(search_display_turn).to receive(:update_range)
    end

    # Command Method -> Test the change in the observable state
    it 'increases guess_count by one' do
      game_display_turn.display_turn_order(search_display_turn)
    ...
    it 'sends make_guess' do
      expect(search_display_turn).to receive(:make_guess).once
      game_display_turn.display_turn_order(search_display_turn)
    end

    # Method with Outgoing Command -> Test that a message is sent
    it 'sends update_range' do
      expect(search_display_turn).to receive(:update_range).once
      game_display_turn.display_turn_order(search_display_turn)
    end
