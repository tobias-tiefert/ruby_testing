...
describe FindNumber do
  ...
  describe '#make_guess' do
    ...
    subject(:guess_five_to_nine) { described_class.new(5, 9, number_guessing) }
    context 'when min is 5 and max is 9' do
      it 'returns 7' do
        expect(guess_five_to_nine.make_guess).to eq(7)
      end
    end

    subject(:guess_eight_to_nine) { described_class.new(8, 9, number_guessing) }
    context 'when min is 8 and max is 9' do
      it 'returns 8' do
        expect(guess_eight_to_nine.make_guess).to eq(8)
      end
    end

    subject(:guess_zero_to_three) { described_class.new(0, 3, number_guessing) }
    context 'when min is 0 and max is 3' do
      it 'returns 1' do
        expect(guess_zero_to_three.make_guess).to eq(1)
      end
    end

    subject(:guess_three_to_three) { described_class.new(3, 3, number_guessing) }
    context 'when min and max both equal 3' do
      it 'returns 3' do
        expect(guess_three_to_three.make_guess).to eq(3)
      end
    end
  end
  ...
  describe '#game_over?' do
    let(:random_number) { double('random_number') }
    subject(:successfull_guess) { described_class.new(0, 9, random_number, 8) }
    context 'when guess and random_number are equal' do
      ...
      it 'is game over' do
        allow(random_number).to receive(:value).and_return(8)
        expect(successfull_guess.game_over?).to be true
      end
    end
    ...
    context 'when guess and random_number are not equal' do
      it 'is not game over' do
        allow(random_number).to receive(:value).and_return(4)
        expect(successfull_guess.game_over?).not_to be true
      end
    end
  end
  ...
  describe '#update_range' do
    ...
    context 'when the guess is 7, min=5, and max=8' do
      subject(:edge_guess) { described_class.new(5, 8, number_range, 7) }
      it 'updates min to the same value as max' do
        edge_guess.update_range
        expect(edge_guess.min).to eq(edge_guess.max)
      end

      it 'does not update max' do
        edge_guess.update_range
        expect(edge_guess.max).to eq(8)
      end
    end
  end
end
